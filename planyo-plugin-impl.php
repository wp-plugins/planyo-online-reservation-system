...
function planyo_output_site_list() {
  ...
  if (planyo_get_param('lang'))
    $language = planyo_get_param('lang');
  $params = array('modver'=>'1.9','site_id'=>"",'metasite_id'=> $planyo_metasite_id, 'mode'=>'display_site_list_code','feedback_url'=>$planyo_feedback_url, 'language'=>$language ? $language : '', 'sort'=>planyo_get_param('sort'), 'cal_filter_name'=>planyo_get_param('cal_filter_name'), 'cal_filter_value'=>planyo_get_param('cal_filter_value'), 'planyo_files_location'=>$planyo_files_location);
  if (is_array($planyo_login_info)) {
    $params['login_cs'] = $planyo_login_info['login_cs'];
    $params['login_email'] = $planyo_login_info['login_email'];
  }
...
var planyo_attribs='<?php if ($planyo_attribs && is_string($planyo_attribs) && planyo_should_use_attribs()) echo '?'.html_entity_decode($planyo_attribs);?>'; // optional attributes for non-default modes
<?php
    if (is_array($planyo_login_info)) {
