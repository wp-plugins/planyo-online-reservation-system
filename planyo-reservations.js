...
function get_form_data(obj) {
  ...
  for (var i=0; i<obj.childNodes.length; i++) {
	...
	if (obj.childNodes[i].checked) {
      ...
      var childstr = get_form_data(obj.childNodes[i]);
...
function get_planyo_mode() {
  ...
  if (get_param ('resource_id') || isset(window.planyo_resource_id)) {
    if (presentation_mode)
...
function form_loaded(code) {
  if (!code || code.substr (0, 5) == "Error") {
    ...
    if (planyo_mode == 'search') {
      ...
      var params_array = new Array('start_date', 'end_date', 'date', 'start_time', 'end_time', 'sort');
	...
	send_search_form();
      ...
      prefill_params(new Array('start_date', 'end_date', 'date', 'start_time', 'end_time'));
    }
  }
}
