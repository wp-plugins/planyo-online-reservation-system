...
function planyo_on_reservation_success(reservation_id, user_text) {
  ...
  window.location.hash = "#res_ok_msg";
}
...
function planyo_on_reservation_failure(error_text) {
   ...
   window.location.hash = "#res_error_msg";
...
function planyo_send_request (data, on_complete_function, hide_item) {
  ...
  if (window.ulap_script.toLowerCase() == 'jsonp') {
      req_data += "&modver=2.6";
      var xhr_req = new Request.JSONP({
          url: (window.planyo_use_https ? "https://" : "http://") + "www.planyo.com/rest/ulap-jsonp.php",
  ...
  else if (planyo_isset(window.Request) && planyo_isset(window.Request.JSON)) {
...
function planyo_on_search_success(results) {
  ...
  eval_children_scripts(planyo$('search_results'));
...
function planyo_form_loaded(code) {
  if (!code || code.substr (0, 5) == "Error") {
    ...
    if (window.ulap_script.toLowerCase() == 'jsonp' || !planyo_isset(window.Request) || !planyo_isset(window.Request.JSON)) {
      eval_children_scripts(planyo$('planyo_content'));
    }
	...
	planyo_send_search_form();
      ...
      if (planyo$('product_form'))
        planyo$('product_form').onsubmit=planyo_send_product_form;
      handle_custom_price_element ();
      window.location.hash = "#planyo_content";
    }
...
function planyo_embed_additional_products_form(reservation_id, ppp_rs) {
  if (reservation_id) {
    var form_url = get_full_planyo_file_path(ulap_script);
    var form_url_params = "ulap_url="+(window.planyo_use_https ? "https" : "http")+"://www.planyo.com/rest/planyo-reservations.php&mode=display_additional_products_code&reservation_id="+reservation_id+(ppp_rs ? ("&ppp_rs=" + ppp_rs) : "")+"&feedback_url="+planyo_get_current_url();
    planyo_embed_code (form_url,form_url_params);
  }
}
...
function planyo_calprev_msg_listener(event) {
  if (event && event.data && event.data.length > 3 && (event.data.substr(0, 3) == 'AVA' || event.data.substr(0, 3) == 'RES' || event.data.substr(0, 3) == 'SIZ')) {
    ...
    if (data_root && event.data.substr(0, 3) == 'RES') {
      ...
      if (planyo$(data_root.iframe_id)) {
        var ifr=planyo$(data_root.iframe_id);
...
function init_planyo() {
  if(!window.MooTools) {
    document.getElementById('planyo_content').innerHTML = "<span id='nolib_error' style='color:red'>Planyo error: MooTools has not been loaded. Either add MooTools to this page or change the plugin settings to include it for you.</span>";
  }
  if (planyo$('planyo_price_holder'))
